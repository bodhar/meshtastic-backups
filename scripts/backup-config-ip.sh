...
# Usage: ./backup-config-ip.sh <ip_address> [backup_name]
# Example: ./backup-config-ip.sh 192.168.1.1 my_backup
...
# If no arguments are provided, prompt for the IP address
if [ "$#" -lt 1 ]; then
  echo "Enter the IP of the Meshtastic node"
  read meshtastic_ip
else
  meshtastic_ip="$1"
fi
...
if ! [[ $meshtastic_ip =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "Invalid IP address format. Please enter a valid IP address."
    exit 1
fi

# Check if a second argument (backup name) was provided
if [ -n "$2" ]; then
  backup_name="$2"
fi
...
if meshtastic --host "$meshtastic_ip" --export-config >"$temp_file"; then
  mkdir -p "$backup_directory"
  # Get the owner name from the config
  device_name=$(grep -w "owner:" "$temp_file" | awk '{print $2}' | tr -d '"')
